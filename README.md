...
The difference is purely additive: the variant carries the same 90 definitions plus five
`PhysicalMedia` parameters — `Alias`, `Enable`, `LastChange`, `LowerLayers` and `Upstream`. No
definition present in the base schema is absent from the variant, and no definition shared by both
